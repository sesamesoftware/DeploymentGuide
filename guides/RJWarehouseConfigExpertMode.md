...
7. Objects Downloaded: Restricted | Unrestricted
   1. Unrestricted is the default setting. 
   2. Objects Downloaded - Restricted 
      1. Use this option to control which tables are downloaded. The objects must exist in the datasource and the user must have access to the objects. Objects cannot be in both the download and exclude config. See the document [Select Table to Load](RJWarehouseConfigSelectTables.md) for more information.
   3. Objects Downloaded - Unrestricted
...
8. Objects Uploaded: Restricted | Unrestricted
   1. Unrestricted is the default setting. 
   2. Objects Downloaded - Restricted 
      1. Use this option to control which tables are uploaded. The objects must exist in the datasource and the user must have access to the objects. Objects need to be in a parent child relational order or else you can get errors like invalid cross reference id. Objects cannot be in both the upload and exclude config. Objects cannot be in both the download and exclude config. See the document [Select Table to Load](RJWarehouseConfigSelectTables.md) for more information.
   3. Objects Downloaded - Unrestricted
...
2.  System of Record: 
    ...
    2.  If set to datasource then when you change records in the database in preparation to send to datasource and a download happens first if the record has changed in the datasource it will overwrite the data in the database.
    3.  If set to Local then when you change records in the database in preparation to send to datasource and a download happens first if the record has changed in datasource the data in the database will not be overwritten as long as their delete_flag is set to any one of the action flags or action flag errors.
